...
txiSMTNeg <- tximport(files, type="salmon", txIn = TRUE, txOut = FALSE, tx2gene=tx2gene_noNA, ignoreTxVersion=TRUE)

# Save files 
save(txiSMTNeg, file = "RNAseq/starting_files/SMT_Neg_matrix_tximport_salmon.Rdata") # objeto R
write.csv(txiSMTNeg$abundance, file = "RNAseq/starting_files/SMT_Neg_matrix_salmon_tximport_abundance.csv") # TPM
write.csv(txiSMTNeg$counts, file = "RNAseq/starting_files/SMT_Neg_matrix_salmon_tximport_counts.csv") # counts

## Run DESeq2
BiocManager::install("EnhancedVolcano")
library(EnhancedVolcano)
library(DESeq2)
metadata <- read.delim("RNAseq/starting_files/Metadata - SMT_Neg_Metadata.tsv", sep="\t")
