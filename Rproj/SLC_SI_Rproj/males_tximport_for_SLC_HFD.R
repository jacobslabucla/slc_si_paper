...
samples <- read.delim("RNAseq/quant_salmon/SLC_HFD_Males.tsv",header=FALSE)
samples <- samples$V1
...
# Save files 
save(txiSLCHFD, file = "RNAseq/starting_files/males_SLC_HFD_matrix_tximport_salmon.Rdata") # objeto R
write.csv(txiSLCHFD$abundance, file = "RNAseq/starting_files/males_SLC_HFD_matrix_salmon_tximport_abundance.csv") # TPM
write.csv(txiSLCHFD$counts, file = "RNAseq/starting_files/males_SLC_HFD_matrix_salmon_tximport_counts.csv") # counts
...
metadata <- metadata %>% filter(Model=="HFD") %>% filter(Sex=="Male")
...
write.csv(CDvsNormMatrix,"RNAseq/males_DESEQ2_HFD_MUT_vs_WT_results.csv")

plot <- read.csv("RNAseq/males_DESEQ2_HFD_MUT_vs_WT_results.csv",row.names = 1)
summary(plot$padj)
plot <- plot %>% filter(padj>1e-7) #remove unannotated outlier
male_hfd_plot <- EnhancedVolcano(plot,
                            lab = rownames(plot),
                            labSize = 6,
                            x = 'log2FoldChange',
                            y = 'padj',
...
male_hfd_plot
ggsave(male_hfd_plot, file="RNAseq/male_hfd_plot.png",width = 7,height=7)

CDvsNormMatrix <- CDvsNormMatrix %>% filter(padj<0.05)
write.csv(CDvsNormMatrix,"RNAseq/males_significant_DESEQ2_HFD_MUT_vs_WT_results.csv")
...
write.csv(CDvsNormMatrix,"RNAseq/males_DESEQ2_HFD_HET_vs_WT_results.csv")

plot <- read.csv("RNAseq/males_DESEQ2_HFD_HET_vs_WT_results.csv",row.names = 1)
summary(plot$padj)
#plot <- plot %>% filter(padj>1e-7) #remove unannotated outlier
het_male_hfd_plot <- EnhancedVolcano(plot,
                                 lab = rownames(plot),
                                 labSize = 6,
                                 x = 'log2FoldChange',
                                 y = 'padj',
  ...
  theme(legend.position = "top")
het_male_hfd_plot
ggsave(het_male_hfd_plot, file="RNAseq/het_male_hfd_plot.png",width = 7,height=7)

CDvsNormMatrix <- CDvsNormMatrix %>% filter(padj<0.05)
write.csv(CDvsNormMatrix,"RNAseq/males_significant_DESEQ2_HFD_HET_vs_WT_results.csv")
